...
function RevenueBreakdown({
    ...
    const items = [
        ...
        },
        {
            ...
            value: grossProfit,
            color: "bg-[#60a5fa]",
            percent: (grossProfit / revenue) * 100,
        },
        {
            label: "Operating Income",
            value: operatingIncome,
            color: "bg-[#a78bfa]",
            percent: (operatingIncome / revenue) * 100,
        },
        {
            label: "Net Income",
            value: netIncome,
            color: "bg-[#3dd68c]",
            percent: (netIncome / revenue) * 100,
        },
    ];
...
export function FinancialsPanel({ symbol }: FinancialsPanelProps) {
    ...
    const income: IncomeStatement =
        incomeData?.annualReports?.[0] || mockData.income;
    const balance: BalanceSheet =
        balanceData?.annualReports?.[0] || mockData.balance;
    const cashFlow: CashFlow =
        cashFlowData?.annualReports?.[0] || mockData.cashFlow;

    // Calculate key ratios
    const grossMargin = income.totalRevenue
        ? income.grossProfit / income.totalRevenue
        : 0;
    const operatingMargin = income.totalRevenue
        ? income.operatingIncome / income.totalRevenue
        : 0;
    const netMargin = income.totalRevenue
        ? income.netIncome / income.totalRevenue
        : 0;
    const debtToEquity = balance.totalShareholderEquity
        ? (balance.totalDebt || 0) / balance.totalShareholderEquity
        : 0;
    const currentRatio = balance.totalLiabilities
        ? balance.totalAssets / balance.totalLiabilities
        : 0;
    const roe = balance.totalShareholderEquity
        ? income.netIncome / balance.totalShareholderEquity
        : 0;
        ...
        <div className="p-5 space-y-4">
            ...
            {activeTab === "balance" && (
                <div className="space-y-4">
                    ...
                    <div className="p-3.5 rounded-xl bg-[#1a1d24]/60 border border-[#2d303a]/30">
                        <h4 className="text-xs font-medium text-[#e8eaed] mb-3">
                            Assets vs Liabilities
                        </h4>
                        <div className="space-y-3">
                            <div className="space-y-1">
                                <div className="flex justify-between text-xs">
                                    ...
                                    <span className="font-mono text-[#c0c4cc]">
                                        {formatLargeNumber(balance.totalAssets)}
                                    </span>
                                </div>
            ...
            {activeTab === "cashflow" && (
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-2.5">
                        <div className="p-3 rounded-xl bg-[#22c55e]/10 border border-[#22c55e]/20">
                            <div className="flex items-center gap-1.5 mb-1">
                                ...
                                <span className="text-[11px] text-[#3dd68c]">
                                    Operating CF
                                </span>
                            </div>
                        ...
                        <div className="p-3 rounded-xl bg-[#ef4444]/10 border border-[#ef4444]/20">
                            <div className="flex items-center gap-1.5 mb-1">
                                ...
                                <span className="text-[11px] text-[#f06c6c]">
                                    CapEx
                                </span>
                            </div>
                    ...
                    <div className="p-4 rounded-xl bg-[#6c8cff]/10 border border-[#6c8cff]/20">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-xs text-[#6c8cff]/80">
                                    Free Cash Flow
                                </p>
                                <p className="text-[11px] text-[#8b8f9a]">
                                    Operating CF - CapEx
                    ...
                    {/* Cash Flow Quality */}
                    <div className="p-3.5 rounded-xl bg-[#1a1d24]/60 border border-[#2d303a]/30">
                        <h4 className="text-xs font-medium text-[#e8eaed] mb-2">
                            Cash Flow Quality
                        </h4>
