...
async function fetchInsiderTrades(
    ...
    const response = await fetch(
        `/api/insider-trading?symbol=${symbol}&limit=${limit}`
    );
...
export function InsiderTrading({
    ...
    const displayTrades = trades?.length
        ? trades
        : generateMockInsiderTrades(symbol);
    ...
    const getTransactionColor = (type: string) => {
        if (type.includes("Purchase") || type.startsWith("P"))
            return "text-[#3dd68c]";
        if (type.includes("Sale") || type.startsWith("S"))
            return "text-[#f06c6c]";
        return "text-[#6c8cff]";
    };
    ...
    const sellVolume = displayTrades
        .filter(
            (t) =>
                t.transactionType.includes("Sale") ||
                t.transactionType.startsWith("S")
        )
        .reduce((acc, t) => acc + t.totalValue, 0);
            ...
            {!compact && (
                <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="p-3 rounded-xl bg-[#f06c6c]/10 border border-[#f06c6c]/20">
                        ...
                        <p className="text-lg font-bold text-[#f06c6c]">
                            {formatValue(sellVolume)}
                        </p>
                    </div>
                </div>
            )}
