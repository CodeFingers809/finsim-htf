...
function generateMockEarnings(symbol: string): EarningsEstimate[] {
    ...
    return quarters.map((quarter, idx) => {
        ...
        const epsEstimate =
            baseEps * (1 + (Math.random() - 0.5) * 0.2) * (1 - idx * 0.02);
        const revenueEstimate =
            baseRevenue * (1 + (Math.random() - 0.5) * 0.1) * (1 - idx * 0.02);
        const epsActual = isReported
            ? epsEstimate * (1 + (Math.random() - 0.3) * 0.15)
            : undefined;
        const revenueActual = isReported
            ? revenueEstimate * (1 + (Math.random() - 0.3) * 0.08)
            : undefined;
        ...
        return {
            ...
            surprisePercent: epsActual
                ? ((epsActual - epsEstimate) / epsEstimate) * 100
                : undefined,
        };
    });
}
            ...
            <div className="p-4 rounded-xl bg-gradient-to-br from-[#6c8cff]/15 to-[#6c8cff]/5 border border-[#6c8cff]/30">
                <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-medium text-[#6c8cff]">
                            Next Earnings
                        </span>
                    </div>
                    <span className="text-xs text-[#8b8f9a]">
                        {upcomingEarnings.quarter}
                    </span>
                </div>
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        <p className="text-xs text-[#8b8f9a] mb-1">
                            EPS Estimate
                        </p>
                ...
                <div className="grid grid-cols-2 gap-3">
                    <div className="p-3 rounded-xl bg-[#3dd68c]/10 border border-[#3dd68c]/20">
                        <div className="flex items-center gap-2 mb-1">
                            ...
                            <span className="text-xs text-[#3dd68c] font-medium">
                                Beats
                            </span>
                        </div>
                        <p className="text-2xl font-bold text-[#3dd68c]">
                            {beats}
                        </p>
                        <p className="text-xs text-[#8b8f9a]">
                            Last {pastEarnings.length} quarters
                        </p>
                    </div>
                    <div className="p-3 rounded-xl bg-[#f06c6c]/10 border border-[#f06c6c]/20">
                        <div className="flex items-center gap-2 mb-1">
                            <XCircle className="h-4 w-4 text-[#f06c6c]" />
                            <span className="text-xs text-[#f06c6c] font-medium">
                                Misses
                            </span>
                        </div>
                        <p className="text-2xl font-bold text-[#f06c6c]">
                            {misses}
                        </p>
                        <p className="text-xs text-[#8b8f9a]">
                            Last {pastEarnings.length} quarters
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="grid grid-cols-2 gap-4 text-xs">
                                        <div>
                                            <p className="text-[#8b8f9a] mb-0.5">
                                                EPS
                                            </p>
                                            <div className="flex items-center gap-2">
                                                <span className="text-[#e8eaed] font-semibold">
                                                    $
                                                    {earning.epsActual?.toFixed(
                                                        2
                                                    )}
                                                </span>
                                                <span className="text-[#8b8f9a]">
                                                    vs $
                                                    {earning?.epsEstimate?.toFixed(
                                                        2
                                                    ) ?? "--"}
                                                </span>
                                            </div>
