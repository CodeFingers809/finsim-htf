...
function getMACDSignal(
    ...
    if (macd === null || signal === null)
        return { signal: "N/A", color: "text-text-secondary" };
    if (macd > signal && macd > 0)
        return { signal: "Strong Buy", color: "text-success" };
    if (macd > signal) return { signal: "Buy", color: "text-success" };
    if (macd < signal && macd < 0)
        return { signal: "Strong Sell", color: "text-danger" };
            ...
            <div className="relative">
                ...
                <div className="h-3 rounded-full bg-[#1e2028] overflow-hidden relative">
                    ...
                    <div className="absolute inset-0 flex">
                        <div className="w-[30%] bg-[#3dd68c]/20" />{" "}
                        {/* Oversold zone */}
                        <div className="w-[40%] bg-[#fbbf24]/10" />{" "}
                        {/* Neutral zone */}
                        <div className="w-[30%] bg-[#f06c6c]/20" />{" "}
                        {/* Overbought zone */}
                    </div>
                    {/* Value indicator */}
                    <div
...
export function TechnicalIndicators({
    ...
    const technicals =
        data?.rsi !== null
            ? data
            : generateMockTechnicals(symbol, currentPrice);
    ...
    return (
        <div className="p-5 space-y-5">
            ...
            <div className="p-3.5 rounded-xl bg-[#1a1d24]/60 border border-[#2d303a]/30 space-y-2.5">
                <div className="flex items-center justify-between">
                    ...
                    <span
                        className={cn(
                            ...
                            macdSignal.color,
                            macdSignal.signal.includes("Buy")
                                ? "bg-[#22c55e]/12"
                                : macdSignal.signal.includes("Sell")
                                ? "bg-[#ef4444]/12"
                                : "bg-[#eab308]/12"
                        )}
                    >
                ...
                <div className="grid grid-cols-3 gap-3">
                    <div>
                        ...
                        <p className="text-xs font-mono text-[#c0c4cc]">
                            {(technicals.macd.macd || 0).toFixed(3)}
                        </p>
                    </div>
                    <div>
                        <p className="text-[11px] text-[#8b8f9a]">Signal</p>
                        <p className="text-xs font-mono text-[#c0c4cc]">
                            {(technicals.macd.signal || 0).toFixed(3)}
                        </p>
                    </div>
                    <div>
            ...
            <div className="space-y-2.5">
                ...
                <div className="grid grid-cols-2 gap-2.5">
                    {[
                        {
                            label: "SMA 20",
                            value: technicals.sma.sma20,
                            signal: getMASignal(
                                currentPrice,
                                technicals.sma.sma20
                            ),
                        },
                        {
                            label: "SMA 50",
                            value: technicals.sma.sma50,
                            signal: sma50Signal,
                        },
                        {
                            label: "SMA 200",
                            value: technicals.sma.sma200,
                            signal: sma200Signal,
                        },
                        {
                            label: "EMA 12",
                            value: technicals.ema.ema12,
                            signal: getMASignal(
                                currentPrice,
                                technicals.ema.ema12
                            ),
                        },
                    ].map((ma, idx) => (
                        <div
                            key={idx}
                            className="p-2.5 rounded-xl bg-[#1a1d24]/60 border border-[#2d303a]/30"
                        >
            ...
            <div className="p-3.5 rounded-xl bg-[#1a1d24]/60 border border-[#2d303a]/30 space-y-2.5">
                ...
                <div className="flex justify-between text-[11px]">
                    <span className="text-[#f06c6c]">
                        L: ${technicals.bollingerBands.lower?.toFixed(2) || "-"}
                    </span>
                    <span className="text-[#8b8f9a]">
                        M: $
                        {technicals.bollingerBands.middle?.toFixed(2) || "-"}
                    </span>
                    <span className="text-[#3dd68c]">
                        U: ${technicals.bollingerBands.upper?.toFixed(2) || "-"}
                    </span>
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2.5">
                ...
                <div className="p-2.5 rounded-xl bg-[#1a1d24]/60 border border-[#2d303a]/30">
                    <p className="text-[11px] text-[#8b8f9a] mb-1">
                        ATR (Volatility)
                    </p>
                    <p className="text-sm font-mono font-semibold text-[#c0c4cc]">
                        ...
                        <div>
                            ...
                            <p
                                className={cn(
                                    ...
                                    (technicals.stochastic.k || 50) > 80
                                        ? "text-[#f06c6c]"
                                        : (technicals.stochastic.k || 50) < 20
                                        ? "text-[#3dd68c]"
                                        : "text-[#c0c4cc]"
                                )}
                            >
